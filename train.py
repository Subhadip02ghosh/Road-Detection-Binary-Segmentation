...
def trainer(model, loader, optimizer, loss_fn, device, epoch):
    ...
    for i, (x, y) in enumerate(loader):
        ...
        data_str = (
            f"\rEpoch: {epoch+1:02} | Iteration: {i+1:02} | Loss: {loss.item():.4f}"
        )
...
def train(model, train_loader, valid_loader, optimizer, checkpoint_path, device):
    ...
    loss_fn = CombinedLoss()  # for Swin-T
    # loss_fn = DiceBCELoss() # for ResNet and SegNet
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0.0:
            ...
            if valid_loss <= best_valid_loss:
                ...
                torch.save(
                    model.state_dict(), os.path.join(checkpoint_path, "ckpt.pth")
                )
...
if __name__ == "__main__":
    seeding(1)  # for Swin-T
    # seeding(42) # for ResNet and SegNet
    ...
    """ Dataset and loader """
    train_dataset = CustomRoadData(
        root_path, "train", transform_img=transforms_img, transform_mask=transforms_mask
    )
